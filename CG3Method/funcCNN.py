...
def CalSupport(A, lam1):
    num1 = np.shape(A)[0]
    # Ensure float type for power operations
    A_ = A + lam1 * np.eye(num1)
    D_ = np.sum(A_, 1)
    D_05 = np.diag(D_.astype(float) ** (-0.5))
...
def normalize_adj(adj):
    ...
    d_inv_sqrt = np.power(rowsum.astype(float), -0.5).flatten()
...
def preprocess_adj(adj):
    """Preprocessing of adjacency matrix."""
...
def LoadData(dataset_str):
    ...
    files_to_check = names + ['test.index']
    
    for name in files_to_check:
        file_name = "ind.{}.{}".format(dataset_str, name)
    ...
    # FIX: Ensure graph size matches features to prevent "inconsistent shapes"
    adj = nx.adjacency_matrix(nx.from_dict_of_lists(graph))
    if adj.shape[0] < features.shape[0]:
        shape_diff = features.shape[0] - adj.shape[0]
        adj = sp.block_diag([adj, sp.csr_matrix((shape_diff, shape_diff))])
...
# ... Rest of your functions (CalSppr, GetKnnAdj, etc.) remain the same ...
